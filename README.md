...
Raw and processed data files are not stored in this repository because many exceed GitHub's 100 MB limit.

- Raw phenotypes are downloaded from T3/Breedbase.
- Genotypes are downloaded from T3 or generated by the pipeline.
- Processed matrices (filtered, imputed, harmonized) are created by scripts in `src/`.

To regenerate data:
1. Run `src/download_data.py`
2. Run `src/preprocess_pheno.py`
3. Run `src/preprocess_geno.py`
4. Run `src/build_grm.py`
